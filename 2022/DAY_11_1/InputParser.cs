...
internal static class InputParser
{
    ...
    private static IEnumerable<long> ParseItems(string line)
    {
        ...
        return s.Split(',').Select(long.Parse);
    }
    ...
    private static Func<long, long> ParseOperation(string line)
    {
        ...
        int.TryParse(operand, out int operandValue);

        return @operator switch
        {
            "+" => (long oldValue) => oldValue + operandValue,
            "*" => (long oldValue) => isUnaryOperation ? oldValue * oldValue : oldValue * operandValue,
            _ => (long _) => 0
        };
    }
